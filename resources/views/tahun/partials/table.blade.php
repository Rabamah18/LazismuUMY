                ...
                <tr class="odd:bg-white odd:dark:bg-gray-800 even:bg-gray-100 even:dark:bg-gray-700">
                    ...
                    <td class="py-4 pl-6 pr-2 lg:pr-4">
                        <div class="flex space-x-2 justify-items-start">
                            <a href="{{ route('tahun.show', $tahun) }}" class="hover:underline">View</a>
                            <a href="{{ route('tahun.edit', $tahun) }}"
                                ...
                                x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion{{ $tahun->id }}')">
                                Hapus
                            </button>

                            <x-modal name="confirm-user-deletion{{ $tahun->id }}" :show="$errors->userDeletion->isNotEmpty()" focusable>
                                <form method="post" action="{{ route('tahun.destroy', $tahun) }}"
                                    class="p-6">
                                    @csrf
                                    @method('delete')

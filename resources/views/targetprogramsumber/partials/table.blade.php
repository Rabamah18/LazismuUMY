                    ...
                    <td class="py-4 pl-6 pr-2 text-center lg:pr-4">
                        <div class="flex justify-center space-x-2">
                            ...
                            <button x-data="" class="text-red-500 hover:underline"
                                x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion{{ $targetProgramSumber->id }}')">
                                Hapus
                            </button>

                            <x-modal name="confirm-user-deletion{{ $targetProgramSumber->id }}" :show="$errors->userDeletion->isNotEmpty()"
                                focusable>
                                <form method="post"
                                    action="{{ route('targetprogramsumber.destroy', $targetProgramSumber) }}"
                                    class="p-6">
                                    @csrf
                                    @method('delete')
